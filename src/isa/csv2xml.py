...
def save_xml(md, schema="mods", output_folder=""):
    parser = etree.XMLParser(remove_blank_text=True)
    file_name = (
        f"{md.pid.split(':')[-1]}.xml"
        if schema == "mods"
        else f"dc-{md.pid.split(':')[-1]}.xml"
    )
    xml = md.to_mods() if schema == "mods" else md.to_dc()
    output = etree.parse(
        BytesIO(etree.tostring(xml, xml_declaration=True, encoding="UTF-8")),
        parser,
    )
...
<oai_dc:dc xmlns:dc="http://purl.org/dc/elements/1.1/"
        ...
        self.corporate_contributor = self._get_concatenated_field(
            "corporate_contributor"
        )
        self.personal_creator_valueURI = self._get_concatenated_field(
            "personal_creator_valueURI"
        )
        self.corporate_creator_valueURI = self._get_concatenated_field(
        ...
        self.personal_contributor_valueURI = self._get_concatenated_field(
            "personal_contributor_valueURI"
        )
        self.corporate_contributor_valueURI = self._get_concatenated_field(
        ...
        self.topical_subject_local = self._get_concatenated_field(
            "topical_subject_local"
        )
        self.geographic_subject_lcsh = self._get_concatenated_field(
            "geographic_subject_lcsh"
        )
        self.geographic_subject_fast = self._get_concatenated_field(
            "geographic_subject_fast"
        )
        self.geographic_subject_local = self._get_concatenated_field(
            "geographic_subject_local"
        )
        self.geographic_subject_geonames = self._get_concatenated_field(
            "geographic_subject_geonames"
        )
        self.coordinates = self._get_concatenated_field("coordinates")
        self.personal_name_subject = self._get_concatenated_field(
            "personal_name_subject"
        )
        self.corporate_name_subject = self._get_concatenated_field(
        ...
        self.topical_subject_lcsh_valueURI = self._get_concatenated_field(
            "topical_subject_lcsh_valueURI"
        )
        self.topical_subject_fast_valueURI = self._get_concatenated_field(
            "topical_subject_fast_valueURI"
        )
        self.topical_subject_local_valueURI = self._get_concatenated_field(
            "topical_subject_local_valueURI"
        )
        self.geographic_subject_lcsh_valueURI = self._get_concatenated_field(
            "geographic_subject_lcsh_valueURI"
        )
        self.geographic_subject_fast_valueURI = self._get_concatenated_field(
            "geographic_subject_fast_valueURI"
        )
        self.geographic_subject_local_valueURI = self._get_concatenated_field(
            "geographic_subject_local_valueURI"
        )
        self.geographic_subject_geonames_valueURI = self._get_concatenated_field(
            "geographic_subject_geonames_valueURI"
        )
        self.personal_name_subject_valueURI = self._get_concatenated_field(
            "personal_name_subject_valueURI"
        )
        self.corporate_name_subject_valueURI = self._get_concatenated_field(
            "corporate_name_subject_valueURI"
        )
        self.birds_subject_valueURI = self._get_concatenated_field(
            "birds_subject_valueURI"
        )
        self.event_subject_valueURI = self._get_concatenated_field(
            "event_subject_valueURI"
        )
        self.chronological_subject = self._get_concatenated_field(
        ...
        self.rights_management_valueURI = self._get_concatenated_field(
    ...
    def _mods_object_title(self, update=True):
        object_title = E.titleInfo(E.title(escape(self.title)))
    ...
    def _mods_physical_collection(self, update=True):
        if (
            ...
            or self.archival_collection
            or self.finding_aid_ark
            or self.physical_location
            or self.archival_series_title
            or self.folder_title
            or self.box
            or self.folder
        ):
            ...
            if self.archival_collection:
                physical_collection.append(
                    E.titleInfo(E.title(escape(self.archival_collection)))
                )
            ...
            if self.physical_location:
                physical_collection.append(
                    E.location(E.physicalLocation(escape(self.physical_location)))
                )
            if self.archival_series_title:
                physical_collection.append(
                    E.relatedItem(
                        E.titleInfo(
                            E.title(escape(self.archival_series_title)),
    ...
    def _mods_digital_collection(self, update=True):
        if self.digital_collection or self.digital_collection_ark:
            digital_collection = E.relatedItem(
                E.titleInfo(E.title(escape(self.digital_collection))),
                E.identifier(
        ...
        if self.related_exhibit or self.related_exhibit_url:
            ...
            if self.related_exhibit:
                digital_exhibit.append(
                    E.titleInfo(E.title(escape(self.related_exhibit)))
                )
    ...
    def _mods_origin_info(self, update=True):
        if (
            self.publisher
            or self.place_of_origin
            or self.date_original
            or self.date_digital
            or self.issuance
            or self.issuance_start
            or self.issuance_end
            or self.frequency
        ):
            ...
            if self.issuance:
                origin_info.append(E.issuance(escape(self.issuance)))
    ...
    def _mods_notes(self, update=True):
        notes = []
        if self.description:
            notes.append(E.abstract(escape(self.description)))
        if self.disclaimer:
            notes.append(
                E.note(
    ...
    def _mods_url(self, update=True):
        if self.url:
            url = E.location(E.url(escape(self.url)))
            if update:
    ...
    def _mods_subject_geographic_elements(
        self, subjects, subject_value_uris, coordinates, authority
    ):
        root = E.subject()
        if authority:
            root.attrib["authority"] = authority
        for sub, uri, coord in zip_longest(
            subjects, subject_value_uris, coordinates, fillvalue=""
        ):
    ...
    def _mods_genre_type(self, update=True):
        ...
        if self._list_not_empty(self.aat_genre):
            genres.extend(
                self._mods_genre_elements(
                    self.aat_genre, self.aat_genre_valueURI, "aat", "genre"
                )
            )
        if self._list_not_empty(self.aat_type):
            genres.extend(
                self._mods_genre_elements(self.aat_type, self.aat_type_valueURI, "aat")
            )
        ...
        if self._list_not_empty(self.dcmi_type):
            genres.extend(
                self._mods_genre_elements(
                    self.dcmi_type, self.dcmi_type_valueURI, "dct"
                )
            )
    ...
    def _mods_rights(self, update=True):
        ...
        if self._list_not_empty(self.rights_management):
            for statement, uri in zip_longest(
                self.rights_management, self.rights_management_valueURI, fillvalue=""
            ):
    ...
    def _mods_record_info(self, update=True):
        if self.date_created or self.date_modified:
    ...
    def _mods_physical_description(self, update=True):
        ...
        if self.extent:
            for extent in self.extent:
                physical_description.append(E.extent(escape(extent)))
        ...
        if self.image_manipulation:
            physical_description.append(
                E.note(escape(self.image_manipulation), type="image-manipulation")
            )
        if self.bits_per_sample:
            physical_description.append(
                E.note(escape(self.bits_per_sample), type="bits-per-sample")
            )
        if self.samples_per_pixel:
            physical_description.append(
                E.note(escape(self.samples_per_pixel), type="samples-per-pixel")
            )
        if self.colorspace:
            physical_description.append(
                E.note(escape(self.colorspace), type="colorspace")
            )
        if self.resolution:
            physical_description.append(
                E.note(escape(self.resolution), type="resolution")
            )
        if self.file_size:
            physical_description.append(
                E.note(escape(self.file_size), type="file-size")
            )
        if self.height:
        ...
        hardware_software = (
            E.note(escape(self.hardware_software), type="hardware/software")
            if self.hardware_software
            else None
        )
    ...
    def _mods_identifiers(self, update=True):
        ...
        if self.project_number:
            identifiers.append(
                E.identifier(escape(self.project_number), type="project-number")
            )
        if self.file_name:
    ...
    def _dc_add_fields(self, element, fields, update=True):
        ...
        for field in fields:
            if field:
                output = e._makeelement(
                    f"{{http://purl.org/dc/elements/1.1/}}{element}",
                    nsmap=self.dc_nsmap,
                )
    ...
    def _dc_add_single_field(self, element, content, update=True):
        ...
        if content:
            item = e._makeelement(
                f"{{http://purl.org/dc/elements/1.1/}}{element}", nsmap=self.dc_nsmap
            )
    ...
    def _dc_publisher(self, update=True):
        return self._dc_add_fields(
            "publisher",
            [self.contributing_institution, self.contributing_institution_valueURI],
            update,
        )
    ...
    def _dc_creator(self, update=True):
        creator_bits = [
            ...
            *self._alternate_fields(
                self.personal_creator, self.personal_creator_valueURI
            ),
            *self._alternate_fields(
                self.corporate_creator, self.corporate_creator_valueURI
            ),
        ]
    ...
    def _dc_contributor(self, update=True):
        contributor_bits = [
            *self._alternate_fields(
                self.personal_contributor, self.personal_contributor_valueURI
            ),
            *self._alternate_fields(
                self.corporate_contributor, self.corporate_contributor_valueURI
            ),
        ]
        return self._dc_add_multiple_paired_fields(
            "contributor", contributor_bits, update
        )
    ...
    def _dc_subject(self, update=True):
        subject_bits = [
            *self._alternate_fields(
                self.topical_subject_lcsh, self.topical_subject_lcsh_valueURI
            ),
            *self._alternate_fields(
                self.topical_subject_fast, self.topical_subject_fast_valueURI
            ),
            *self._alternate_fields(
                self.topical_subject_local, self.topical_subject_local_valueURI
            ),
            ...
            *self._alternate_fields(
                self.personal_name_subject, self.personal_name_subject_valueURI
            ),
            *self._alternate_fields(
                self.corporate_name_subject, self.corporate_name_subject_valueURI
            ),
        ]
    ...
    def _dc_coverage(self, update=True):
        coverage_bits = [
            *self._alternate_fields(
                self.geographic_subject_lcsh, self.geographic_subject_lcsh_valueURI
            ),
            *self._alternate_fields(
                self.geographic_subject_fast, self.geographic_subject_fast_valueURI
            ),
            *self._alternate_fields(
                self.geographic_subject_local, self.geographic_subject_local_valueURI
            ),
            *self._alternate_fields(
    ...
    def _dc_rights(self, update=True):
        rights_bits = self._alternate_fields(
            self.rights_management_valueURI, self.rights_management
        )
